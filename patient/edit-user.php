...
// Check email collision (another patient already owns this email)
$chk = $database->prepare("SELECT pid FROM patient WHERE pemail = ? AND pid != ?");
...
$profile_image_sql = '';   // empty = no change to image column
if (isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {
    ...
    if ($file['size'] <= $max_size && in_array($mime, $allowed_types) && in_array($ext, $allowed_exts)) {
        ...
        // Delete old image
        $old = $database->prepare("SELECT profile_image FROM patient WHERE pid = ?");
        $old->bind_param('i', $id);
        $old->execute();
...
$sql = "UPDATE patient
        SET pemail='$email', pname='$name', ppassword='$password',
            ptel='$tele', paddress='$address'
            $profile_image_sql
        WHERE pid=$id";
$database->query($sql);
